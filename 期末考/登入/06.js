...
function spanValue(spanID, spanValue) { //匹配失败时，修改后面字的总和
	switch(spanValue) {
		case "usernameSpan":
			spanID.firstChild.nodeValue = "6-16位：英文.数字";
			break;
		case "passwordSpan":
			spanID.firstChild.nodeValue = "6-16位";
			break;
		case "repasswordSpan":
			spanID.firstChild.nodeValue = "請輸入密碼";
			break;
		case "repasswordSpan1":
			spanID.firstChild.nodeValue = "密碼不一樣";
			break;
		case "usernameAgain":
			spanID.firstChild.nodeValue = "註冊成功！";
	}
}
...
function submit() { //对提交按钮进行设置
	...
	submit.onclick = function() {
		if(usernamenum && passwordnum && repasswordnum && vercodenum) { //如果各项内容都填写正确，写入cookie
			...
			//alert(tempCookie);
			cookie = username.value + "username" + "=" + username.value + ";" + username.value + "password" + "=" + password.value + ";" + username.value + ";" + tempCookie;
